<?php
    ...
    */
    ...
    $weblog_minical_sql = "SELECT title,blog_id,`created`,contents FROM ".$db->prefix($plugin['dirname']). " as bl," . $db->prefix('users') . " as u," . $db->prefix('groups_users_link') . " as g WHERE `created` >= $range_start_s AND `created` < $range_end_s and private!='Y' AND bl.user_id=u.uid AND bl.user_id=g.uid " . $add_whr . $group_by ;
    $result = $db->query($weblog_minical_sql) ;
    while( list( $title , $blog_id , $server_time , $description ) = $db->fetchRow( $result ) ) {
        ...
        $tmp_array = array(
            ...
            'description' => $myts->displayTarea( strip_tags( $description ) )
        ) ;

...
function b_xhnewbb_new_show($options) {
    ...
    $query='SELECT t.topic_id, t.topic_title, t.topic_last_post_id, t.topic_time, t.topic_views, t.topic_replies, t.forum_id, f.forum_name FROM '.$db->prefix('xhnewbb_topics').' t, '.$db->prefix('xhnewbb_forums').' f WHERE f.forum_id=t.forum_id AND f.forum_type <> 1 ORDER BY '.$order.' DESC';
...
//            if ( $options[1] != 0 ) {
			...
			// Ryuji_edit(2003-11-11) hack start
			    ...
			    $topic['poster'] =  $tmpuser;
				$topic['last_subject'] = $myts->makeTboxData4Show($tmpdb['subject']);
...
function b_xhnewbb_new_private_show($options) {
    $db =& Database::getInstance();
    $myts =& MyTextSanitizer::getInstance();
    $block = array();
    switch($options[2]) {
    ...
    while ($arr = $db->fetchArray($result)) {
        ...
        $lastpostername = $db->query("SELECT post_id, uid FROM ".$db->prefix("xhnewbb_posts")." WHERE post_id = $tmpuser2");
        while ($tmpdb=$db->fetchArray($lastpostername)) {
            $tmpuser = XoopsUser::getUnameFromId($tmpdb['uid']);
            if ( $options[1] != 0 ) {
                $topic['time'] = formatTimestamp($arr['topic_time'],'m')." $tmpuser";
...
function b_xhnewbb_new_edit($options) {
    $inputtag = "<input type='text' name='options[0]' value='".$options[0]."' />";
    $form = sprintf(_MB_XHNEWBB_DISPLAY,$inputtag);
    $form .= "<br />"._MB_XHNEWBB_DISPLAYF."&nbsp;<input type='radio' name='options[1]' value='1'";
    if ( $options[1] == 1 ) {

<?php
// $Id: notification.inc.php,v 1.2 2005/02/10 19:05:45 gij Exp $
//  ------------------------------------------------------------------------ //
...
function xhnewbb_notify_iteminfo($category, $item_id)
{
	...
	if ($category=='forum') {
		...
		$sql = 'SELECT forum_name FROM ' . $xoopsDB->prefix('xhnewbb_forums') . ' WHERE forum_id = '.$item_id;
	...
	if ($category=='thread') {
		...
		$sql = 'SELECT t.topic_title,f.forum_id,f.forum_name FROM '.$xoopsDB->prefix('xhnewbb_topics') . ' t, ' . $xoopsDB->prefix('xhnewbb_forums') . ' f WHERE t.forum_id = f.forum_id AND t.topic_id = '. $item_id . ' limit 1';
		$result = $xoopsDB->query($sql); // TODO: error check
		$result_array = $xoopsDB->fetchArray($result);
	...
	if ($category=='post') {
		...
		$sql = 'SELECT subject,topic_id,forum_id FROM ' . $xoopsDB->prefix('xhnewbb_posts') . ' WHERE post_id = ' . $item_id . ' LIMIT 1';

<?php

//%%%%%%		Module Name phpBB  		%%%%%



// Appended by Xoops Language Checker -GIJOE- in 2006-08-07 17:50:20
define('_MD_XHNEWBB_ALT_ICON1','normal');
define('_MD_XHNEWBB_ALT_ICON2','unhappy');
define('_MD_XHNEWBB_ALT_ICON3','happy');
define('_MD_XHNEWBB_ALT_ICON4','raise it');
define('_MD_XHNEWBB_ALT_ICON5','lower it');
define('_MD_XHNEWBB_ALT_ICON6','report');
define('_MD_XHNEWBB_ALT_ICON7','question');
define('_MD_XHNEWBB_ALT_SOLVED','(solved)');
define('_MD_XHNEWBB_ALT_UNSOLVED','(unsolved)');

// Appended by Xoops Language Checker -GIJOE- in 2006-08-06 06:01:43
define('_MD_XHNEWBB_UPPERTOPIC','top of the topics');
define('_MD_XHNEWBB_LOWERTOPIC','bottom of the topics');

define("_MD_XHNEWBB_UPDATE","更新");
define("_MD_XHNEWBB_UPDATED","已更新了");


//functions.php
define("_MD_XHNEWBB_ERROR","錯誤");
define("_MD_XHNEWBB_NOPOSTS","尚無文章");
define("_MD_XHNEWBB_GO","送出");

//index.php
define("_MD_XHNEWBB_FORUM","討論區");
define("_MD_XHNEWBB_WELCOME","歡迎來到 %s 討論區.");
define("_MD_XHNEWBB_TOPICS","主題數");
define("_MD_XHNEWBB_POSTS","發表數");
define("_MD_XHNEWBB_LASTPOST","最後發表");
define("_MD_XHNEWBB_MODERATOR","版主");
define("_MD_XHNEWBB_NEWPOSTS","有新未讀文章");
define("_MD_XHNEWBB_NONEWPOSTS","無未讀的文章");
define("_MD_XHNEWBB_PRIVATEFORUM","私人討論區");
define("_MD_XHNEWBB_BY","發表者："); // Posted by
define("_MD_XHNEWBB_TOSTART","選擇您要瀏覽的討論版，並針對該討論版性質正確的發表或討論。");
define("_MD_XHNEWBB_TOTALTOPICSC","所有主題數：");
define("_MD_XHNEWBB_TOTALPOSTSC","所有文章數：");
define("_MD_XHNEWBB_TIMENOW","現在時間是 %s");
define("_MD_XHNEWBB_LASTVISIT","您上次來訪是：%s");
define("_MD_XHNEWBB_ADVSEARCH","進階搜尋");
define("_MD_XHNEWBB_POSTEDON","發表時間：");
define("_MD_XHNEWBB_SUBJECT","主旨：");

//page_header.php
define("_MD_XHNEWBB_MODERATEDBY","版主");
define("_MD_XHNEWBB_SEARCH","搜尋");
define("_MD_XHNEWBB_SEARCHRESULTS","搜尋結果");
define("_MD_XHNEWBB_FORUMINDEX","討論區主頁");
define("_MD_XHNEWBB_ALLTOPICSINDEX","主題一覽");
define("_MD_XHNEWBB_POSTNEW","發表新主題");
define("_MD_XHNEWBB_REGTOPOST","請先登入或是註冊再發表！");

//search.php
define("_MD_XHNEWBB_KEYWORDS","關鍵字:");
define("_MD_XHNEWBB_SEARCHANY","任一條件符合 (預設值)");
define("_MD_XHNEWBB_SEARCHALL","全部條件符合");
define("_MD_XHNEWBB_SEARCHALLFORUMS","搜尋所有討論區");
define("_MD_XHNEWBB_FORUMC","討論版:");
define("_MD_XHNEWBB_SORTBY","排序:");
define("_MD_XHNEWBB_DATE","日期");
define("_MD_XHNEWBB_TOPIC","主題");
define("_MD_XHNEWBB_USERNAME","帳號");
define("_MD_XHNEWBB_SEARCHIN","搜尋於:");
define("_MD_XHNEWBB_BODY","內容");
define("_MD_XHNEWBB_NOMATCH","沒有符合的，請再換個條件試試.");
define("_MD_XHNEWBB_POSTTIME","發表時間");

//viewforum.php
define("_MD_XHNEWBB_LATESTPOST","最新");
define("_MD_XHNEWBB_REPLIES","回應");
define("_MD_XHNEWBB_POSTER","發表者");
define("_MD_XHNEWBB_VIEWS","閱\覽");
define("_MD_XHNEWBB_MORETHAN","最新熱門文章");
define("_MD_XHNEWBB_MORETHAN2","最新熱門文章");
define("_MD_XHNEWBB_TOPICLOCKED","本主題已經鎖定，停止回應");
define("_MD_XHNEWBB_LEGEND","Legend");
define("_MD_XHNEWBB_NEXTPAGE","下一頁");
define("_MD_XHNEWBB_SORTEDBY","排序：");
define("_MD_XHNEWBB_FORUMNAME","討論區名");
define("_MD_XHNEWBB_TOPICTITLE","主題");
define("_MD_XHNEWBB_NUMBERREPLIES","回應數");
define("_MD_XHNEWBB_TOPICPOSTER","原作者");
define("_MD_XHNEWBB_LASTPOSTTIME","最終發表");
define("_MD_XHNEWBB_ASCENDING","升羃排列");
define("_MD_XHNEWBB_DESCENDING","降羃排列");
define("_MD_XHNEWBB_FROMLASTDAYS","過去 %s 天以來");
define("_MD_XHNEWBB_THELASTYEAR","這一年份");
define("_MD_XHNEWBB_BEGINNING","所有主題");
define("_MD_XHNEWBB_WHRSOLVED","Q&A 狀態：");
define("_MD_XHNEWBB_SOLVEDNO","未解決");
define("_MD_XHNEWBB_SOLVEDYES","已解決");
define("_MD_XHNEWBB_SOLVEDBOTH","全面");

//viewtopic.php
define("_MD_XHNEWBB_AUTHOR","發表者");
define("_MD_XHNEWBB_LOCKTOPIC","鎖定回應");
define("_MD_XHNEWBB_UNLOCKTOPIC","解除鎖定");

define("_MD_XHNEWBB_MOVETOPIC","移動主題");
define("_MD_XHNEWBB_DELETETOPIC","刪除主題");
define("_MD_XHNEWBB_TOP","頁首");
define("_MD_XHNEWBB_PARENT","發表於");
define("_MD_XHNEWBB_PREVTOPIC","前一個主題");
define("_MD_XHNEWBB_NEXTTOPIC","下一個主題");

define("_MD_XHNEWBB_MARK_TURNON","將這個主題標示起來");
define("_MD_XHNEWBB_MARK_TURNOFF","取消這個主題的標示");

//forumform.inc
define("_MD_XHNEWBB_ABOUTPOST","發表限制");
define("_MD_XHNEWBB_ANONCANPOST","<b>非會員</b> 可以發表");
define("_MD_XHNEWBB_PRIVATE","<b>私人會員</b> 可以發表");
define("_MD_XHNEWBB_REGCANPOST","<b>已註冊會員</b>可以發表");
define("_MD_XHNEWBB_MODSCANPOST","<B>版主或站長</b>可以發表");
define("_MD_XHNEWBB_PREVPAGE","上一頁");
define("_MD_XHNEWBB_QUOTE","引文");

// ERROR messages
define("_MD_XHNEWBB_ERRORFORUM","錯誤！未選擇討論區");
define("_MD_XHNEWBB_ERRORPOST","錯誤！未無選擇文章");
define("_MD_XHNEWBB_NORIGHTTOPOST","你無權在本區張貼文章。");
define("_MD_XHNEWBB_NORIGHTTOACCESS","您沒有權限進入本討論區.");
define("_MD_XHNEWBB_ERRORTOPIC","錯誤！未選擇主題");
define("_MD_XHNEWBB_ERRORCONNECT","錯誤！無法聯結到討論區資料庫");
define("_MD_XHNEWBB_ERROREXIST","錯誤！你所選擇的討論版並不存在，請再試一次。");
define("_MD_XHNEWBB_ERROROCCURED","發生錯誤！");
define("_MD_XHNEWBB_COULDNOTQUERY","無法反查討論區資料。");
define("_MD_XHNEWBB_FORUMNOEXIST","錯誤！你所選擇的主題/討論區並不存在，請再試一次。");
define("_MD_XHNEWBB_USERNOEXIST","使用者並不存在。請在搜尋一遍。");
define("_MD_XHNEWBB_COULDNOTREMOVE","錯誤！無法移除文章！");
define("_MD_XHNEWBB_COULDNOTREMOVETXT","錯誤！無法移除文章內容！");

//reply.php
define("_MD_XHNEWBB_ON","發表時間：");
define("_MD_XHNEWBB_USERWROTE","%s 寫道：");

//post.php
define("_MD_XHNEWBB_FORMTITLEINPREVIEW","由預覽狀態再發表");
define("_MD_XHNEWBB_EDITNOTALLOWED","您無修改文章的權限！");
define("_MD_XHNEWBB_EDITEDBY","修改紀錄：");
define("_MD_XHNEWBB_ANONNOTALLOWED","非會員無法發表文章。<br>請您先註冊後登入發表。");
define("_MD_XHNEWBB_THANKSSUBMIT","謝謝您的發表！");
define("_MD_XHNEWBB_THANKSEDIT","內容已經更新完畢。");
define("_MD_XHNEWBB_REPLYPOSTED","您發表的文章有人回應了。");
define("_MD_XHNEWBB_HELLO"," %s 您好，");
define("_MD_XHNEWBB_URRECEIVING","您會收到本封mail是因為您在 %s 的討論區發表的文章有人回應了。");

define("_MD_XHNEWBB_CLICKBELOW","按以下連結觀看文章：");
define("_MD_XHNEWBB_FMT_GUESTSPOSTHEADER","由訪客「%s」所發表的文章。\n---\n\n");

//forumform.inc
define("_MD_XHNEWBB_YOURNAME","您的名稱：");
define("_MD_XHNEWBB_LOGOUT","登出");
define("_MD_XHNEWBB_REGISTER","註冊或登入");
define("_MD_XHNEWBB_SUBJECTC","標題：");
define("_MD_XHNEWBB_EDITMODEC","編輯模式：");
define("_MD_XHNEWBB_ALERTEDIT","<div style='background:#FE0000;color:#FFFFFF;font-size:150%;font-weight:bold;padding:3px;'>正在編輯發表文章內容</div>");
define("_MD_XHNEWBB_GUESTNAMEC","訪客稱呼：");
define("_MD_XHNEWBB_UNAMEC","會員名稱：");
define("_MD_XHNEWBB_FMT_UNAME"," %s ");
define("_MD_XHNEWBB_MESSAGEICON","文章圖示：");
define("_MD_XHNEWBB_SOLVEDCHECKBOX","已解決");
define("_MD_XHNEWBB_MESSAGEC","內容：");
define("_MD_XHNEWBB_ALLOWEDHTML","可使用的HTML標籤：");
define("_MD_XHNEWBB_OPTIONS","附加選項：");
define("_MD_XHNEWBB_POSTANONLY","匿名發表");
define("_MD_XHNEWBB_DISABLESMILEY","不使用表情圖");
define("_MD_XHNEWBB_DISABLEHTML","不使用 html 標籤");
define("_MD_XHNEWBB_NEWPOSTNOTIFY","本討論串有發表時通知我");
define("_MD_XHNEWBB_ATTACHSIG","附加簽名");
define("_MD_XHNEWBB_POST","發表");
define("_MD_XHNEWBB_SUBMIT","確定");
define("_MD_XHNEWBB_CANCELPOST","取消");

// forumuserpost.php
define("_MD_XHNEWBB_ADD","增加");
define("_MD_XHNEWBB_REPLY","對這文章發表回應");

// topicmanager.php
define("_MD_XHNEWBB_YANTMOTFTYCPTF","你並非本區版主所以並不能使用此功\能。");
define("_MD_XHNEWBB_TTHBRFTD","這主題已經被移除了。");
define("_MD_XHNEWBB_RETURNTOTHEFORUM","回到討論版列表");
define("_MD_XHNEWBB_RTTFI","回討論區主頁");
define("_MD_XHNEWBB_EPGBATA","錯誤！請再試一遍。");
define("_MD_XHNEWBB_TTHBM","這主題已經移往別版。");
define("_MD_XHNEWBB_VTUT","觀看修改成果");
define("_MD_XHNEWBB_TTHBL","這主題已經鎖住停止回應了。");

define("_MD_XHNEWBB_VIEWTHETOPIC","觀看該主題");
define("_MD_XHNEWBB_TTHBU","這主題已經解開鎖定，恢復回應。");
define("_MD_XHNEWBB_OYPTDBATBOTFTTY","按下刪除按鍵將使有關討論一起被刪除。");
define("_MD_XHNEWBB_OYPTMBATBOTFTTY","按下移動按鍵將使有關討論一起移動到你所選的地方");
define("_MD_XHNEWBB_OYPTLBATBOTFTTY","按下鎖定按鍵將使有關討論一起被鎖定回應。除非你按下解除鎖定才能開放回應。");
define("_MD_XHNEWBB_OYPTUBATBOTFTTY","按下解除鎖定按鍵將可重新開放，如果要重新鎖定的話必須重新按下鎖定按鍵。");
define("_MD_XHNEWBB_MOVETOPICTO","移動主題到：");
define("_MD_XHNEWBB_NOFORUMINDB","無討論區於資料庫中");
define("_MD_XHNEWBB_DATABASEERROR","資料庫錯誤");
define("_MD_XHNEWBB_DELTOPIC","刪除文章");
define("_MD_XHNEWBB_TOPICSTICKY","主題已經固定在本版頂端");
define("_MD_XHNEWBB_STICKYTOPIC","固定本主題於本版頂端");
define("_MD_XHNEWBB_UNSTICKYTOPIC","解除固定主題");
define("_MD_XHNEWBB_TTHBS","這個主題已經固定在頂端。");
define("_MD_XHNEWBB_TTHBUS","這個主題已經解除固定！");
define("_MD_XHNEWBB_OYPTSBATBOTFTTY","當您按下按鈕可以固定主題在頂端.");
define("_MD_XHNEWBB_OYPTTBATBOTFTTY","當您按下按鈕可以解除固定主題.");

// delete.php
define("_MD_XHNEWBB_DELNOTALLOWED","抱歉！您並沒有權限刪除本篇文章！");
define("_MD_XHNEWBB_AREUSUREDEL","您確定要刪除本篇文章及以下的回應？");
define("_MD_XHNEWBB_POSTSDELETED","選擇本篇及其以下文章一起刪除。");

// definitions moved from global.
define("_MD_XHNEWBB_THREAD","討論內容");
define("_MD_XHNEWBB_FROM","來自");
define("_MD_XHNEWBB_JOINED","註冊日");
define("_MD_XHNEWBB_ONLINE","<font color=red><b>在線上！</b></font>");
define("_MD_XHNEWBB_BOTTOM","頁尾");

// multi-byte spaces separated by ,
// (don't define for single space languages)
define('_MD_XHNEWBB_MULTIBYTESPACES','　') ;

?>
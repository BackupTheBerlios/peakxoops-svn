	...
	function checkReserveStatus($eid)// 0:予約受付なし、1:予約受付中、2:予約申込み済、3:予約受付終了
	{
		...
		if($ret_n > 0 && is_object($user))
		{
			while($data = $this->db->fetchArray($ret))
			{
				...
				$this->reserve['rdate'] = strftime('%Y/%m/%d %H:%M',$data['rdate']);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function explodeInfo($info_array, $opt_array)
	{
		...
		{
//echo 'info:'.mb_convert_encoding($info_array[$i],HTML_CODE,SCRIPT_CODE).'<br />';
//echo 'opt:'.mb_convert_encoding($opt_array[$i],HTML_CODE,SCRIPT_CODE).'<br />';
//echo 'preg_match:'.mb_convert_encoding("/^!(".$info_array[$i].")$/",HTML_CODE,SCRIPT_CODE).'<br />';

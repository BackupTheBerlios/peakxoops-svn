...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobileNewbbPlugin extends XmobilePlugin
{
	function XmobileNewbbPlugin()
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobileNewbbPluginHandler extends XmobilePluginHandler
{
//	var $moduleDir = 'newbb';
//	var $categoryTableName = 'bb_topics';
//	var $itemTableName = 'bb_posts';
...
// データ詳細は丸ごとHTMLでitem_detailとして出力
	function getDetailView()
	{
		...
		$sql = "SELECT DISTINCT p.post_id, p.subject, p.post_time, p.uid, p.nohtml, p.nosmiley, f.cat_id, t.topic_id, f.forum_id, f.cat_id FROM ".$this->db->prefix('bb_posts')." p LEFT JOIN ".$this->db->prefix('bb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('bb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
		...
		while($row = $this->db->fetchArray($ret))
		{
			...
			$sql = 'SELECT post_text FROM '.$this->db->prefix('bb_posts_text').' WHERE post_id = '.$this->item_id;
			// debug
		...
		$sql = 'UPDATE '.$this->db->prefix('bb_topics').' SET topic_views=topic_views+1 WHERE topic_id='.$this->topic_id;
		// debug
	...
	function getItemList()
	{
		...
		switch($this->levelState)
		{
			case 'category':
				...
				$sql = "SELECT DISTINCT c.cat_id, c.cat_title, SUM(f.forum_posts) AS post_count FROM ".$this->db->prefix('bb_categories')." c INNER JOIN ".$this->db->prefix('bb_forums')." f ON c.cat_id=f.cat_id LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON a.forum_id=f.forum_id WHERE ".$this->item_criteria->render()." GROUP BY f.cat_id ORDER BY c.cat_order";
			...
			case 'forum':
				...
				$sql = "SELECT DISTINCT f.forum_id, f.forum_name, f.forum_posts as post_count FROM ".$this->db->prefix('bb_forums')." f LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON a.forum_id=f.forum_id WHERE (".$this->item_criteria->render().") AND f.cat_id=".$this->cat_id." ORDER BY f.forum_id";
				$this->setNextViewState('list');
				// $extraの値はgetLinkUrl()でhtmlspecialchars()を掛けられるので&amp;ではなく&と記述しておく
			...
			case 'topic':
				...
				$sql = "SELECT DISTINCT t.topic_id, t.topic_title, COUNT(p.post_id) AS post_count FROM ".$this->db->prefix('bb_posts')." p LEFT JOIN ".$this->db->prefix('bb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('bb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.forum_id=".$this->forum_id." GROUP BY p.topic_id ORDER BY t.topic_time ".$this->item_order_sort;
				$this->setNextViewState('list');
				// $extraの値はgetLinkUrl()でhtmlspecialchars()を掛けられるので&amp;ではなく&と記述しておく
			...
			case 'post':
				...
				$sql = "SELECT DISTINCT p.post_id, p.subject, p.post_time, p.uid FROM ".$this->db->prefix('bb_posts')." p LEFT JOIN ".$this->db->prefix('bb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('bb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND p.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
...
//		$this->setItemCriteria();
		...
		$sql = 'SELECT DISTINCT f.cat_id, p.forum_id, p.topic_id, p.post_id, p.subject, p.post_time, p.uid FROM '.$this->db->prefix('bb_posts').' p LEFT JOIN '.$this->db->prefix('bb_topics').' t ON p.topic_id=t.topic_id LEFT JOIN '.$this->db->prefix('bb_forums').' f ON t.forum_id=f.forum_id LEFT JOIN '.$this->db->prefix('bb_forum_access').' a ON f.forum_id=a.forum_id WHERE '.$this->item_criteria->render().' ORDER BY p.post_time DESC';
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function checkForumAccess()
	{
		...
		if($this->forum_id)
		{
			$sql = 'SELECT forum_access,forum_type FROM '.$this->db->prefix('bb_forums').' WHERE forum_id = '.$this->forum_id;
		}
		elseif($this->topic_id)
		{
			$sql = 'SELECT f.forum_access,f.forum_type FROM '.$this->db->prefix('bb_topics').' t LEFT JOIN '.$this->db->prefix('bb_forums').' f ON t.forum_id=f.forum_id WHERE t.topic_id='.$this->topic_id;
		}
		else
		{
		...
		if($forum_type == 1) // private forum
		{
			if(is_object($user))
			{
				...
				$sql3 = 'SELECT user_id, can_post FROM '.$this->db->prefix('bb_forum_access').' WHERE forum_id = '.$this->forum_id.' AND user_id = '.$uid;
				if(!$ret3 = $this->db->query($sql3))
				{
					// debug
					$this->utils->setDebugMessage(__CLASS__, 'checkForumAccess bb_forum_access sql', $sql3);
					$this->utils->setDebugMessage(__CLASS__, 'checkForumAccess bb_forum_access sql error', $this->db->error());
				}
		...
		if($uid)
		{
			$sql4 = 'SELECT post_id FROM '.$this->db->prefix('bb_posts').' WHERE uid = '.$uid.' AND post_id = '.$this->post_id;
			if(!$ret4 = $this->db->query($sql4))
			{
				// debug
				$this->utils->setDebugMessage(__CLASS__, 'checkForumAccess bb_posts sql', $sql4);
				$this->utils->setDebugMessage(__CLASS__, 'checkForumAccess bb_posts sql error', $this->db->error());
			}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function getNicePathFromId($sel_id, $level, $path='')
	{
		...
		switch ($level)
		{
			...
			case 'forum':
				$sql = 'SELECT cat_id, cat_title FROM '.$this->db->prefix('bb_categories').' WHERE cat_id='.$sel_id;
			...
			case 'topic':
				$sql = 'SELECT cat_id, forum_name FROM '.$this->db->prefix('bb_forums').' WHERE forum_id='.$sel_id;
			...
			case 'post':
				$sql = 'SELECT forum_id, topic_title FROM '.$this->db->prefix('bb_topics').' WHERE topic_id='.$sel_id;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		$sql = "SELECT DISTINCT p.post_id FROM ".$this->db->prefix('bb_posts')." p LEFT JOIN ".$this->db->prefix('bb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('bb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('bb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
		// debug
	...
	function renderEntryForm()
	{
		...
		switch($entry_type)
		{
			...
			case 'reply_entry':
				...
				$nohtml = 0;
				$nosmiley = 0;
				$sql = "SELECT p.subject, p.post_time, p.uid, p.nohtml, p.nosmiley , pt.post_text FROM ".$this->db->prefix('bb_posts')." p LEFT JOIN ".$this->db->prefix('bb_posts_text')." pt ON p.post_id=pt.post_id WHERE p.post_id = ".$this->post_id;
				$ret = $this->db->query($sql);
				if(!$ret)
				{
					// debug
				...
				while($data = $this->db->fetchArray($ret))
				{
					...
					$rep_post_text = $myts->makeTareaData4Show($data['post_text'],$nohtml,$nosmiley,1);
				}
			...
			case 'edit_entry':
				...
				$sql = 'SELECT subject FROM '.$this->db->prefix('bb_posts').' WHERE post_id = '.$this->post_id;
				$ret = $this->db->query($sql);
				if(!$ret)
				{
					// debug
				...
				while($data = $this->db->fetchArray($ret))
				{
					$subject = $myts->makeTboxData4Show($data['subject']);
				}
				$sql2 = 'SELECT post_text FROM '.$this->db->prefix('bb_posts_text').' WHERE post_id = '.$this->post_id;
				...
				{
					$post_text = $myts->makeTareaData4Edit($data2['post_text']);
				}
			...
			case 'delete_entry':
				...
				$sql_has_child = 'SELECT post_id FROM '.$this->db->prefix('bb_posts').' WHERE pid = '.$this->post_id;
				$ret_has_child = $this->db->query($sql_has_child);
				$child_count = $this->db->getRowsNum($ret_has_child);

				$sql = 'SELECT subject, post_time, uid, nohtml, nosmiley FROM '.$this->db->prefix('bb_posts').' WHERE post_id = '.$this->post_id;
				$ret = $this->db->query($sql);
				if(!$ret)
				{
					// debug
...
//			return $this->ticket->getErrors();
		...
		$sql = 'SELECT allow_html FROM '.$this->db->prefix('bb_forums').' WHERE forum_id = '.$this->forum_id;
		$ret = $this->db->query($sql);
		if(!$ret)
		{
		...
		switch($entry_type)
		{
			case 'new_entry':
				...
				$sql_insert_topics = "INSERT INTO ".$this->db->prefix('bb_topics')." (topic_title,topic_poster,topic_time,topic_views,topic_replies,topic_last_post_id,forum_id) VALUES ('$subject',$uid,$post_time,0,0,0,".$this->forum_id.")";
				...
				$sql_insert_posts = "INSERT INTO ".$this->db->prefix('bb_posts')." (pid,topic_id,forum_id,post_time,uid,poster_ip,subject,nohtml,nosmiley,icon,attachsig) VALUES($pid,$new_topic_id,$this->forum_id,$post_time,$uid,'$poster_ip','$subject',$nohtml,$nosmiley,'$icon',$attachsig)";
				// debug
				...
				$sql_insert_posts_text = "INSERT INTO ".$this->db->prefix('bb_posts_text')." (post_id,post_text) VALUES($new_id,'$post_text')";
				...
				{
					$sql_delete_topics = "DELETE FROM ".$this->db->prefix('bb_topics')." WHERE topic_id = $new_topic_id";
					$this->db->query($sql_delete_topics);

					$sql_delete_posts = "DELETE FROM ".$this->db->prefix('bb_posts')." WHERE post_id = $new_id";
					$this->db->query($sql_delete_posts);
					$this->utils->setDebugMessage(__CLASS__, 'sql_delete_posts error', $this->db->error());
					return _MD_XMOBILE_INSERT_FAILED;
				}

				$sql_update_forums = "UPDATE ".$this->db->prefix('bb_forums')." SET forum_topics = forum_topics+1, forum_posts = forum_posts+1, forum_last_post_id = $new_id WHERE forum_id = $this->forum_id";
				...
				$sql_update_topics= "UPDATE ".$this->db->prefix('bb_topics')." SET topic_last_post_id = $new_id WHERE topic_id = $new_topic_id";
			...
			case 'reply_entry':
				...
				$sql_insert_posts = "INSERT INTO ".$this->db->prefix('bb_posts')." (pid,topic_id,forum_id,post_time,uid,poster_ip,subject,nohtml,nosmiley,icon,attachsig) VALUES($pid,$this->topic_id,$this->forum_id,$post_time,$uid,'$poster_ip','$subject',$nohtml,$nosmiley,'$icon',$attachsig)";
				$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts', $sql_insert_posts);
				if(!$ret_insert_posts = $this->db->query($sql_insert_posts))
				{
					$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts error', $this->db->error());
				...
				if(!$ret_insert_posts_text = $this->db->query($sql_insert_posts_text))
				{
					$sql_delete_posts = "DELETE FROM ".$this->db->prefix('bb_posts')." WHERE post_id = $this->post_id";
					$this->db->query($sql_delete_posts);
					$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts_text error', $this->db->error());
					return false;
				}

				$sql_update_topics = "UPDATE ".$this->db->prefix('bb_topics')." SET topic_poster=".$uid.", topic_time=".$post_time.", topic_replies=topic_replies+1, topic_last_post_id=".$new_id." WHERE topic_id=".$this->topic_id;
				$this->utils->setDebugMessage(__CLASS__, 'sql_update_topics', $sql_update_topics);
				if(!$ret_update_topics = $this->db->query($sql_update_topics))
				{
					$this->utils->setDebugMessage(__CLASS__, 'sql_update_topics error', $this->db->error());
				...
				$sql_update_forums = "UPDATE ".$this->db->prefix('bb_forums')." SET forum_posts = forum_posts+1, forum_last_post_id = $new_id WHERE forum_id = $this->forum_id";
			...
			case 'edit_entry':
				...
				$sql_insert_posts = "UPDATE ".$this->db->prefix('bb_posts')." SET pid=$pid, topic_id=".$this->topic_id.", forum_id=".$this->forum_id.",post_time=$post_time,poster_ip='$poster_ip',subject='$subject',nohtml=$nohtml,nosmiley=$nosmiley,icon='$icon',attachsig=$attachsig WHERE post_id=".$this->post_id;
				$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts', $sql_insert_posts);
				if(!$ret_insert_posts = $this->db->query($sql_insert_posts))
				{
					$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts error', $this->db->error());
				...
				$sql_insert_posts_text = "UPDATE ".$this->db->prefix('bb_posts_text')." SET post_id=".$this->post_id.",post_text='$post_text' WHERE post_id=".$this->post_id;
				$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts_text', $sql_insert_posts_text);
				if(!$ret_insert_posts_text = $this->db->query($sql_insert_posts_text))
				{
					$this->utils->setDebugMessage(__CLASS__, 'sql_insert_posts_text error', $this->db->error());
				...
				$sql_insert_topics = "UPDATE ".$this->db->prefix('bb_topics')." SET topic_time=$post_time WHERE topic_id=".$this->topic_id;
				$this->utils->setDebugMessage(__CLASS__, 'sql_insert_topics', $sql_insert_topics);
				if(!$ret_insert_topics = $this->db->query($sql_insert_topics))
				{
					$this->utils->setDebugMessage(__CLASS__, 'sql_insert_topics error', $this->db->error());
			...
			case 'delete_entry':
				...
				$poster_uid = 0;
				$sql_posts = "SELECT uid FROM ".$this->db->prefix('bb_posts')." WHERE post_id = ".$this->post_id;
				...
				$istopic = 0;
				$pid = 0;
				$sql_istopic = 'SELECT pid FROM '.$this->db->prefix('bb_posts').' WHERE post_id = '.$this->post_id;
				...
				$sql_delete_posts_text = "DELETE FROM ".$this->db->prefix('bb_posts_text')." WHERE post_id = ".$this->post_id;
				...
				if($child_count > 0)
				{
					...
					{
						$sql_delete_posts_child = "DELETE FROM ".$this->db->prefix('bb_posts')." WHERE pid=".$child['post_id'];
						...
						$sql_delete_posts_text_child = "DELETE FROM ".$this->db->prefix('bb_posts_text')." WHERE post_id=".$child['post_id'];
				...
				if($istopic)
				{
					$sql_delete_topics = "DELETE FROM ".$this->db->prefix('bb_topics')." WHERE topic_id=".$this->topic_id;
...
*/
					$sql_update_forum_topics = "UPDATE ".$this->db->prefix('bb_forums')." SET forum_topics=forum_topics-1 WHERE forum_id=".$this->forum_id;
				...
				else
				{
					...
					$topic_time = 0;
					$sql_topic_last_post_id = "SELECT post_id,post_time FROM ".$this->db->prefix('bb_posts')." WHERE topic_id=".$this->topic_id." ORDER BY post_time ".$this->item_order_sort;
					...
					$sql_update_topics = "UPDATE ".$this->db->prefix('bb_topics')." SET topic_time=".$topic_time.", topic_replies=topic_replies-".$post_count.", topic_last_post_id=".$topic_last_post_id." WHERE topic_id=".$this->topic_id;
					$this->utils->setDebugMessage(__CLASS__, 'sql_update_topics', $sql_update_topics);
					if(!$ret_update_topics = $this->db->query($sql_update_topics))
					{
						$this->utils->setDebugMessage(__CLASS__, 'sql_update_topics error', $this->db->error());
				...
				$sql_forum_last_post_id = "SELECT post_id FROM ".$this->db->prefix('bb_posts')." WHERE forum_id=".$this->forum_id." ORDER BY post_time ".$this->item_order_sort;
				...
				$sql_update_forum_posts = "UPDATE ".$this->db->prefix('bb_forums')." SET forum_posts=forum_posts-".$post_count.", forum_last_post_id=".$forum_last_post_id." WHERE forum_id=".$this->forum_id;

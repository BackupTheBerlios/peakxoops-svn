...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobilePicalPlugin extends XmobilePlugin
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobilePicalPluginHandler extends XmobilePluginHandler
{
//	var $moduleDir = 'piCal';
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function addItemCriteria()
	{
		...
		// 開始日が選択した年月
		$start_date = mktime(0,0,0,$this->month,1,$this->year,0);
		$end_date = mktime(0,0,0,$this->month+1,1,$this->year,0) -1;
		$criteria_date =& new CriteriaCompo();
		$criteria_date->add(new Criteria('start', $start_date, '>='));
		$criteria_date->add(new Criteria('end', $end_date, '<='));

		$this->item_criteria->add($criteria_date);
		// debug
		$this->utils->setDebugMessage(__CLASS__, 'start_date', strftime('%Y-%m-%d %H:%M',$start_date));
//		$this->utils->setDebugMessage(__CLASS__, 'end_date', strftime('%Y-%m-%d %H:%M',$end_date));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function setDete()
	{
		...
		if(is_null($item_id))
		{
			$this->year = intval($this->utils->getGetPost('year', strftime('%Y',time())));
			$this->month = intval($this->utils->getGetPost('month', strftime('%m',time())));
		}
		else
		{
			...
			else
			{
				...
				$this->year = intval($this->utils->getGetPost('year', strftime('%Y',$start)));
				$this->month = intval($this->utils->getGetPost('month', strftime('%m',$start)));
			}
		}
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//		$this->utils->setDebugMessage(__CLASS__, 'getCatSelect criteria', $criteria->render());
	...
	// year and month select
		$select .= '<select name="year">'._MD_XMOBILE_YEAR;
		for($i=-1;$i<=1;$i++)
		{
			$temp_year = strftime('%Y',time()) + $i;
			...
			$select .= '<option value="'.$temp_year.'" '.$sel.'>'.$temp_year;
		}
		$select .= '</select>';

		$select .= '<select name="month">'._MD_XMOBILE_MONTH;
		for($i=1;$i<=12;$i++)
		{
			if($i == $this->month)
			{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function renderCalendar($year=0,$month=0)
	{
		...
		$previous_month_link = $_SERVER['PHP_SELF'].'?'.$QUERY_STRING.$previous_month_par;
		$next_month_link = $_SERVER['PHP_SELF'].'?'.$QUERY_STRING.$next_month_par;
	
		$this_month_link = $_SERVER['PHP_SELF'].'?'.$QUERY_STRING.'&amp;month='.$month.'&amp;year='.$year.'&amp;';

...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobileXhnewbbPlugin extends XmobilePlugin
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class XmobileXhnewbbPluginHandler extends XmobilePluginHandler
{
...
// データ詳細は丸ごとHTMLでitem_detailとして出力
	function getDetailView()
	{
		...
		$sql = "SELECT DISTINCT p.post_id, p.subject, p.post_time, p.uid, p.nohtml, p.nosmiley, f.cat_id, t.topic_id, f.forum_id, f.cat_id FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('xhnewbb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
		...
		while($row = $this->db->fetchArray($ret))
		{
			...
			$sql = 'SELECT post_text FROM '.$this->db->prefix('xhnewbb_posts_text').' WHERE post_id = '.$this->item_id;
			// debug
		...
		$sql = 'UPDATE '.$this->db->prefix('xhnewbb_topics').' SET topic_views=topic_views+1 WHERE topic_id='.$this->topic_id;
		// debug
	...
	function getItemList()
	{
		...
		switch($this->levelState)
		{
			case 'category':
				...
				$sql = "SELECT DISTINCT c.cat_id, c.cat_title, SUM(f.forum_posts) AS post_count FROM ".$this->db->prefix('xhnewbb_categories')." c INNER JOIN ".$this->db->prefix('xhnewbb_forums')." f ON c.cat_id=f.cat_id LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON a.forum_id=f.forum_id WHERE ".$this->item_criteria->render()." GROUP BY f.cat_id ORDER BY c.cat_order";
			...
			case 'forum':
				...
				$sql = "SELECT DISTINCT f.forum_id, f.forum_name, f.forum_posts as post_count FROM ".$this->db->prefix('xhnewbb_forums')." f LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON a.forum_id=f.forum_id WHERE (".$this->item_criteria->render().") AND f.cat_id=".$this->cat_id." ORDER BY f.forum_weight";
				$this->setNextViewState('list');
				// $extraの値はgetLinkUrl()でhtmlspecialchars()を掛けられるので&amp;ではなく&と記述しておく
			...
			case 'topic':
				...
				$sql = "SELECT DISTINCT t.topic_id, t.topic_title, COUNT(p.post_id) AS post_count FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('xhnewbb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.forum_id=".$this->forum_id." GROUP BY p.topic_id ORDER BY t.topic_time ".$this->item_order_sort;
				$this->setNextViewState('list');
				// $extraの値はgetLinkUrl()でhtmlspecialchars()を掛けられるので&amp;ではなく&と記述しておく
			...
			case 'post':
				...
				$sql = "SELECT DISTINCT p.post_id, p.subject, p.post_time, p.uid FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('xhnewbb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND p.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
...
// ただし、戻り値はオブジェクトではなく配列
	function getRecentList()
	{
		...
		$sql = 'SELECT DISTINCT f.cat_id, p.forum_id, p.topic_id, p.post_id, p.subject, p.post_time, p.uid FROM '.$this->db->prefix('xhnewbb_posts').' p LEFT JOIN '.$this->db->prefix('xhnewbb_topics').' t ON p.topic_id=t.topic_id LEFT JOIN '.$this->db->prefix('xhnewbb_forums').' f ON t.forum_id=f.forum_id LEFT JOIN '.$this->db->prefix('xhnewbb_forum_access').' a ON f.forum_id=a.forum_id WHERE '.$this->item_criteria->render().' ORDER BY p.post_time '.$this->item_order_sort;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function getNicePathFromId($sel_id, $level, $path='')
	{
		...
		switch($level)
		{
			...
			case 'forum':
				$sql = 'SELECT cat_id, cat_title FROM '.$this->db->prefix('xhnewbb_categories').' WHERE cat_id='.$sel_id;
			...
			case 'topic':
				$sql = 'SELECT cat_id, forum_name FROM '.$this->db->prefix('xhnewbb_forums').' WHERE forum_id='.$sel_id;
			...
			case 'post':
				$sql = 'SELECT forum_id, topic_title FROM '.$this->db->prefix('xhnewbb_topics').' WHERE topic_id='.$sel_id;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		$sql = "SELECT DISTINCT p.post_id FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_topics')." t ON p.topic_id=t.topic_id LEFT JOIN ".$this->db->prefix('xhnewbb_forums')." f ON t.forum_id=f.forum_id LEFT JOIN ".$this->db->prefix('xhnewbb_forum_access')." a ON f.forum_id=a.forum_id WHERE (".$this->item_criteria->render().") AND t.topic_id=".$this->topic_id." ORDER BY p.post_time ".$this->item_order_sort;
		// debug
	...
	function renderEntryForm()
	{
		...
		switch($entry_type)
		{
			...
			case 'reply_entry':
				...
				$nohtml = 0;
				$nosmiley = 0;
				$sql = "SELECT p.subject, p.post_time, p.uid, p.nohtml, p.nosmiley , pt.post_text FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_posts_text')." pt ON p.post_id=pt.post_id WHERE p.post_id = ".$this->post_id;
				$ret = $this->db->query($sql);
				if(!$ret)
				{
					// debug
			...
			case 'edit_entry':
				...
				$sql = "SELECT p.subject, p.uid, p.nosmiley , pt.post_text FROM ".$this->db->prefix('xhnewbb_posts')." p LEFT JOIN ".$this->db->prefix('xhnewbb_posts_text')." pt ON p.post_id=pt.post_id WHERE p.post_id = ".$this->post_id;
...
//				$entry_form .= '<input type="hidden" name="nohtml" value="'.$nohtml.'" />';
			...
			case 'delete_entry':
				...
				$sql_has_child = 'SELECT post_id FROM '.$this->db->prefix('xhnewbb_posts').' WHERE pid = '.$this->post_id;
				$ret_has_child = $this->db->query($sql_has_child);
				$child_count = $this->db->getRowsNum($ret_has_child);

				$sql = 'SELECT subject, post_time, uid, nohtml, nosmiley FROM '.$this->db->prefix('xhnewbb_posts').' WHERE post_id = '.$this->post_id;
				$ret = $this->db->query($sql);
				if(!$ret)
				{
					// debug
				...
				$sql2 = 'SELECT post_text FROM '.$this->db->prefix('xhnewbb_posts_text').' WHERE post_id = '.$this->post_id;

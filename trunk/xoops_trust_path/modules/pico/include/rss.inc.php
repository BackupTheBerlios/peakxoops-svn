...
while( $content_row = $db->fetchArray( $result ) ) {
	$content4assign_tmp = array(
		...
		'body4rss' => htmlspecialchars( xoops_substr( strip_tags( pico_filter_body( $mydirname , $content_row , $content_row['use_cache'] ) ) , 0 , 255 ) , ENT_QUOTES ) ,
		'created_time_formatted' => formatTimestamp( $content_row['created_time'] ) ,
		'created_time4rss' => formatTimestamp( $content_row['created_time'] , 'rss' ) ,
		'modified_time_formatted' => formatTimestamp( $content_row['modified_time'] ) ,
		'modified_time4rss' => formatTimestamp( $content_row['modified_time'] , 'rss' ) ,

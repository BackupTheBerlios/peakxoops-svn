...
define("_MB_PICO_CATLIMITDSC","※複数指定する時はカテゴリー番号をカンマ(,)で区切る。正数指定の場合は、サブカテゴリーは含まないので必要なら、各サブカテゴリーを明示的に指定する。0はトップカテゴリーを意味する。カテゴリーを指定しない時は空欄にする。マイナス指定の場合は、その絶対値カテゴリーと子孫カテゴリーを意味する");
...
define("_MB_PICO_TAGS","タグ (複数指定なら+や*で区切る）");

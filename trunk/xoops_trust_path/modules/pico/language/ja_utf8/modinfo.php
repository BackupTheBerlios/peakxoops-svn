...
define($constpref.'_INHERITCONFIGS','上位カテゴリによるオプション変更値を継承する');
define($constpref.'_INHERITCONFIGSDSC','1.7/1.8系と動作コンパチにするなら「いいえ」にしてください');
...
define($constpref.'_TPL_VIEWCONTENT','コンテンツ表示用テンプレート');
define($constpref.'_TPL_VIEWCONTENTDSC','初期値は(dirname)_viewcontent.html');
define($constpref.'_TPL_CONTENT_FORM','コンテンツ編集用テンプレート');
define($constpref.'_TPL_CONTENT_FORMDSC','初期値は(dirname)_content_form.html');
define($constpref.'_TPL_CATEGORY_FORM','カテゴリ編集用テンプレート');
define($constpref.'_TPL_CATEGORY_FORMDSC','初期値は(dirname)_category_form.html');
define($constpref.'_TPL_LISTCONT_FORM','カテゴリ内コンテンツ一覧表示用テンプレート');
define($constpref.'_TPL_LISTCONT_FORMDSC','初期値は(dirname)_listcontents.html');
...
define($constpref.'_EF_CLASS' , 'extra_fields処理クラス名（コンテンツ用）');
define($constpref.'_EF_CLASSDSC' , 'extra_fields処理をオーバーライドしたい時に指定。デフォルトはPicoExtraFields');
define($constpref.'_EFCAT_CLASS' , 'extra_fields処理クラス名（カテゴリ用））');
define($constpref.'_EFCAT_CLASSDSC' , 'extra_fields処理をオーバーライドしたい時に指定。デフォルトはPicoExtraFieldsCat');
...
define($constpref.'_EFIMAGES_SIZEDSC' , '(解像度0横幅)x(解像度0高さ)-(品質) (解像度1横幅)x(解像度1高さ)-(品質) ... というフォーマットで記入。解像度指定は何段階でも可能。デフォルトは 480x480-65 150x150-65 の２段階。');
...
define($constpref.'_EF_SORTABLES' , 'ソート可能フィールド');
define($constpref.'_EF_SORTABLESDSC' , 'ソート対象となり得るextra_fieldsのフィールド名をカンマで区切って列挙 例) country,email,height');
